...
## Syllabus-To-CBLM Mapping Rule

Use this mapping exactly:

- Syllabus Learning Outcome -> CBLM Unit of Competency
- Syllabus Topic under that Learning Outcome -> CBLM Learning Outcome
- Syllabus Subtopic under that Topic -> CBLM Content item

In short:
- LO -> UC
- Topic -> LO
- Subtopic -> Contents (`Key Facts`, `Let's Exercise`, `Let's Apply`)

Therefore:
- each syllabus learning outcome produces one separate CBLM DOCX
- inside that CBLM, each topic becomes one LO section
- inside that LO section, each subtopic becomes one content block
- after all unit-level CBLM DOCX files are completed, combine them into `CBLM_FULL.docx`
...
Interpret the structured payload using the syllabus mapping above:
- `current_unit` = one syllabus learning outcome
- `current_unit.learning_outcomes[]` = topics under that syllabus learning outcome
- `current_unit.learning_outcomes[].contents[]` = subtopics under that topic
...
### Key Facts
- One Key Facts per Topic (Learning Outcome)
- Minimum 1800 words per Topic Key Facts
...
### Let's Apply
- one activity block per Topic (Learning Outcome), based on the syllabus Activity for that Topic
...
- validate each Topic Key Facts section has at least 1800 words
...
## Anti-Template Writing Rule

When authoring Key Facts or other long instructional text:
- do NOT generate prose through local Python phrase banks, variant arrays, slot-filling text builders, or other synthetic content generators
- do NOT create helper scripts in `./state` that mass-produce Key Facts from reusable sentence patterns
- do NOT switch from direct drafting into a generic generator just because the text is long
- if a terminal command would become too long, write the payload file mechanically only after the actual content has already been authored in this run
- a local script may serialize or save already-authored content, but it must not be used to invent the instructional prose itself

If the model is about to rely on a programmatic prose generator to finish the unit:
- stop cleanly
- save only valid intermediate state
- report that continuation is required for manual-quality drafting
- wait for `continue`
...
2. Treat each syllabus learning outcome as one Unit of Competency
3. For the selected syllabus learning outcome, treat each topic as one CBLM Learning Outcome
4. For each topic in that selected syllabus learning outcome, treat each subtopic as one content item
5. Build structured qualification data
6. For the current unit only, create one draft structured payload JSON
7. Save the draft payload JSON in `./state`
8. Validate the payload and similarity constraints
9. If valid, assemble one DOCX for the current unit only
10. Save generated files in `./output`
11. After all units for the syllabus are completed, generate ONE IA for the full syllabus (course-level IA) that lists all contents across all generated CBLM units, and save outputs under `./output/ia` and `./state/ia_payloads`
12. Pause and report status after each completed unit unless explicit instructions say to continue automatically
13. Move the source syllabus into `./processed` only after the full IA and any requested exams are completed

Before marking a unit failed for Key Facts redundancy:
- verify that the draft was authored directly for the current unit and was not mass-generated by a local prose-builder script
- if the failed draft came from a synthetic local generator pattern, do not move the source file to `./failed` yet
- instead, keep the source file in `./inbox`, keep the payload in `./state`, report the problem, and wait for `continue`
...
For every unit payload, `current_unit.module_title` must be a gerund-style phrasing of `current_unit.unit_of_competency` (almost the same meaning, but the first word is an `-ing` form).

Example:
- UC: `Apply Quality Standards`
